...
-- ── ② 라리뷰 후기를 잠근다 ─────────────────────────────────
--    ⚠️ 먼저 앱에서 제목을 '3.' 으로 시작하게 맞춰둘 것. 제목이 다르면 이 update 는
--       0건을 바꾸고 에러 없이 조용히 지나간다(실제로 한 번 겪었다). ③ 으로 꼭 확인한다.
...
-- 사용법
-- ============================================================
-- ℹ️ 잠금은 후기의 id 에 붙는다. 그래서 한 번 잠근 뒤에는 앱에서 제목·글·사진을 아무리
--    고쳐도 잠김이 유지된다(is_template 과 같은 방식).
--    다만 위 SQL 은 제목으로 대상을 찾으므로, 순서는 "앱에서 제목 확정 → SQL 로 잠금" 이다.
--
