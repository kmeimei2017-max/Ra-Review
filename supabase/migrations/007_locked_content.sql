...
--
-- ⚠️ 아래 설명은 008 에서 바뀌었다. 지금 동작은 008_lock_ra_review.sql 을 볼 것.
-- 잠긴 후기는 보내기·삭제 버튼이 둘 다 화면에서 사라진다
-- (사진 안에 이미 버튼 그림이 있어서 겹쳐 보이기 때문).
-- 좌우 스와이프 이동(좌=편집, 우=홈)은 잠긴 후기에서도 그대로 되고, 막히는 건 삭제뿐이다.
